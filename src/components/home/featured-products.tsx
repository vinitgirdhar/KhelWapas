...
export default function FeaturedProducts() {
  ...
  useEffect(() => {
    ...
    const fetchProducts = async () => {
      ...
      try {
        ...
        console.log('[FeaturedProducts] API Response:', { success: data.success, count: data.products?.length });
        if (data.success && data.products) {
          setAllProducts(data.products);
          console.log('[FeaturedProducts] Products loaded:', data.products.length);
        } else {
          console.error('[FeaturedProducts] Invalid API response:', data);
        }
      } catch (error) {
        console.error('[FeaturedProducts] Failed to fetch products:', error);
      } finally {
        setLoading(false);
      }
    };
  ...
  useEffect(() => {
    if (!mounted || allProducts.length === 0) {
      console.log('[FeaturedProducts] Skipping filter:', { mounted, productsCount: allProducts.length });
      return;
    }
    
    setLoading(true);
    console.log('[FeaturedProducts] Filtering products:', { category: activeCategory, totalProducts: allProducts.length });
    // Simulate loading for better UX
    const timer = setTimeout(() => {
      if (activeCategory === 'All') {
        setFilteredProducts(allProducts.slice(0, 6));
        console.log('[FeaturedProducts] Showing all products (first 6)');
      } else {
        const filtered = allProducts.filter((p) => p.category === activeCategory);
        setFilteredProducts(filtered);
        console.log('[FeaturedProducts] Filtered products:', { category: activeCategory, count: filtered.length });
      }
      setLoading(false);
    }, 300);
